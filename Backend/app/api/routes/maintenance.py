...
def list_maintenance(field_id: int | None = Query(None, gt=0), facility_id: int | None = Query(None, gt=0),
                     ...
                     date_to: date | None = None, user: User = Depends(require_owner),
...
def maintenance_summary(user: User = Depends(require_owner), db: Session = Depends(get_db)):
    return MaintenanceService(db).summary(user)


@router.post('', response_model=MaintenanceResponse, status_code=201)
def create_maintenance(payload: MaintenanceCreate, user: User = Depends(require_owner), db: Session = Depends(get_db)):
    return MaintenanceService(db).create(payload, user)


@router.put('/{maintenance_id}', response_model=MaintenanceResponse)
def update_maintenance(maintenance_id: int, payload: MaintenanceUpdate, user: User = Depends(require_owner), db: Session = Depends(get_db)):
...
def start_maintenance(maintenance_id: int, user: User = Depends(require_owner), db: Session = Depends(get_db)):
...
def complete_maintenance(maintenance_id: int, user: User = Depends(require_owner), db: Session = Depends(get_db)):
...
def cancel_maintenance(maintenance_id: int, user: User = Depends(require_owner), db: Session = Depends(get_db)):
