...
from ...services.password_reset_service import GENERIC, create_challenge, verify_challenge
...
def application_response(item: OwnerApplication) -> OwnerApplicationResponse:
    return OwnerApplicationResponse(
        ...
        withdraw_reason=item.withdraw_reason,
...
def _latest_application(db: Session, customer_id: int) -> OwnerApplication | None:
    return db.scalar(
        ...
        .where(OwnerApplication.customer_id == customer_id)
        .order_by(OwnerApplication.created_at.desc(), OwnerApplication.id.desc())
        .limit(1)
    )
...
def _validate_application_data(payload: OwnerApplicationRequest):
    ...
    missing = [key for key in required_rep if not representative.get(key)] + [
        key for key in required_venue if not venue.get(key)
    ]
    if missing:
        raise HTTPException(
            status_code=422,
            detail=f'Hồ sơ còn thiếu thông tin bắt buộc: {", ".join(missing)}',
        )
...
def login(payload: LoginRequest, db: Session = Depends(get_db)):
    ...
    return TokenResponse(
        access_token=create_access_token(
            {'sub': str(user.id), 'role': user.role, 'sv': user.session_version}
        ),
...
def refresh_session(payload: RefreshTokenRequest, db: Session = Depends(get_db)):
    ...
    if result.rowcount != 1:
        db.rollback()
        raise HTTPException(
            status_code=401, detail='Refresh token đã hết hiệu lực hoặc đã được sử dụng'
        )
    db.commit()
    db.refresh(user)
    return TokenResponse(
...
def request_owner(
    payload: OwnerApplicationRequest,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    if current_user.role != UserRole.CUSTOMER.value:
        raise HTTPException(
            status_code=409, detail='Chỉ CUSTOMER mới có thể gửi yêu cầu trở thành OWNER'
        )
    ...
    if item.status in (OwnerApplicationStatus.PENDING.value, OwnerApplicationStatus.APPROVED.value):
        raise HTTPException(
            status_code=409,
            detail='Hồ sơ đang chờ xét duyệt hoặc đã được phê duyệt; không thể gửi trùng',
        )
    if item.status == OwnerApplicationStatus.WITHDRAWN.value:
        raise HTTPException(
            status_code=409, detail='Hồ sơ đã rút không thể gửi lại; hãy tạo hồ sơ đăng ký mới'
        )
    item.status = OwnerApplicationStatus.PENDING.value
    item.representative = payload.representative.model_dump()
    item.venue = payload.venue.model_dump()
    item.legal_confirmed = True
    item.rejection_reason = None
    item.admin_note = None
    item.reviewed_by = None
    item.reviewed_at = None
    item.withdrawn_at = None
    item.withdraw_reason = None
    ...
    try:
        ...
        db.rollback()
        logger.exception(
            'Database error while customer_id=%s submitted owner application id=%s',
            current_user.id,
            item.id,
        )
        raise HTTPException(
            status_code=500,
            detail='Không thể lưu hồ sơ do lỗi hệ thống. Vui lòng thử lại sau.',
        )
    return application_response(item)


@router.put('/owner-application', response_model=OwnerApplicationResponse)
def save_owner_application(
    payload: OwnerApplicationDraft,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    if current_user.role != UserRole.CUSTOMER.value:
    ...
    if item is None:
        item = OwnerApplication(
            ...
            status=OwnerApplicationStatus.DRAFT.value,
            submitted_at=datetime.now(timezone.utc),
        )
        db.add(item)
    elif item.status not in (
        OwnerApplicationStatus.DRAFT.value,
        OwnerApplicationStatus.REJECTED.value,
    ):
        raise HTTPException(
            status_code=409, detail='Trạng thái hồ sơ hiện tại không cho phép chỉnh sửa'
        )
    item.representative = payload.representative
    item.venue = payload.venue
    item.legal_confirmed = payload.legal_confirmed
    db.commit()
    db.refresh(item)
    return application_response(item)
...
@router.post(
    '/owner-application/{application_id}/withdraw', response_model=OwnerApplicationResponse
)
def withdraw_owner_application(
    application_id: int,
    payload: OwnerApplicationWithdraw,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    if current_user.role != UserRole.CUSTOMER.value:
    ...
    if result.rowcount != 1:
        ...
        raise HTTPException(
            status_code=409, detail='Trạng thái hồ sơ hiện tại không cho phép rút'
        )
    db.expire_all()
    ...
    record_audit(
        ...
        {
            'to_status': OwnerApplicationStatus.WITHDRAWN.value,
            'reason': reason,
        },
    )
    db.commit()
...
@router.post(
    '/owner-application/reapply',
    response_model=OwnerApplicationResponse,
    status_code=status.HTTP_201_CREATED,
)
def reapply_owner_application(
    current_user: User = Depends(get_current_user), db: Session = Depends(get_db)
):
    if current_user.role != UserRole.CUSTOMER.value:
    ...
    if previous is None or previous.status != OwnerApplicationStatus.WITHDRAWN.value:
        raise HTTPException(
            status_code=409, detail='Chỉ có thể đăng ký lại sau khi hồ sơ gần nhất đã được rút'
        )
    item = OwnerApplication(
        customer_id=current_user.id,
        status=OwnerApplicationStatus.DRAFT.value,
        representative=dict(previous.representative or {}),
        venue=dict(previous.venue or {}),
...
def update_profile(
    ...
    ensure_unique(
        db, current_user.email, changes.get('phone', current_user.phone), current_user.id
    )
...
def _request_reset(destination: str, channel: str, db: Session):
    normalized = destination.lower().strip() if channel == 'email' else destination.strip()
    user = db.scalar(
        select(User).where(
            (User.email == normalized if channel == 'email' else User.phone == normalized),
    ...
    code = create_challenge(db, user.id, normalized, channel)
    try:
        if channel == 'email':
        ...
        logger.warning('Reset OTP delivery configuration/runtime warning for %s (%s): %s', normalized, channel, exc)
        raise HTTPException(status_code=503, detail=str(exc))
    except Exception as exc:
        logger.exception('Reset OTP delivery failed for %s (%s)', normalized, channel)
        channel_name = 'email' if channel == 'email' else 'tin nhắn SMS'
        raise HTTPException(
            status_code=503,
            detail=f'Không thể gửi mã xác nhận qua {channel_name} lúc này. Vui lòng thử lại sau.',
        )
    return {'message': GENERIC}
...
def forgot_password_email(payload: ForgotPasswordEmailRequest, db: Session = Depends(get_db)):
    return _request_reset(payload.email, 'email', db)
...
def forgot_password_phone(payload: ForgotPasswordPhoneRequest, db: Session = Depends(get_db)):
    return _request_reset(payload.phone, 'phone', db)
...
def verify_otp_endpoint(payload: VerifyOTPRequest, db: Session = Depends(get_db)):
    destination = payload.identifier.lower().strip() if payload.channel == 'email' else payload.identifier.strip()
    user = db.scalar(
        select(User).where(
            (User.email == destination if payload.channel == 'email' else User.phone == destination),
            User.is_active.is_(True),
        )
    )
    if not user:
        raise HTTPException(status_code=400, detail='Mã xác nhận không đúng hoặc đã hết hạn.')
    verify_challenge(db, user.id, destination, payload.channel, payload.otp)
    return {
