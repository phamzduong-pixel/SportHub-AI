import { AlertCircle, Bot, CalendarDays, ChevronRight, Clock3, MapPin, Mic, MicOff, Plus, Send, ShieldCheck, Sparkles, Star, UserRound, WalletCards, X } from 'lucide-react';
...
const createDefaultWelcomeMessage = (courtId?: number): Message => ({
  id: 1,
  role: 'assistant',
  text: courtId
...
export function AIAssistantPage() {
  ...
  const [messages, setMessages] = useState<Message[]>([createDefaultWelcomeMessage(initialCourtId)]);
  ...
  return (
    <div className="min-h-[calc(100vh-4rem)] bg-[radial-gradient(circle_at_top_left,_#ecfdf5,_transparent_35%),#f8fafc] px-3 py-5 sm:px-6 sm:py-8">
      <div className="mx-auto grid max-w-6xl gap-5 lg:grid-cols-[280px_minmax(0,1fr)]">
        <aside className="hidden rounded-3xl bg-brand-900 p-6 text-white shadow-xl lg:flex lg:flex-col">
          <div className="grid h-12 w-12 place-items-center rounded-2xl bg-brand-500">
            <Sparkles size={23} />
          </div>
          ...
          <div className="mt-8 space-y-3 text-sm">
            <p className="flex gap-3">
              <CalendarDays className="text-brand-400" size={18} /> Tìm theo ngày và giờ
            </p>
            <p className="flex gap-3">
              <MapPin className="text-brand-400" size={18} /> Lọc khu vực phù hợp
            </p>
            <p className="flex gap-3">
              <WalletCards className="text-brand-400" size={18} /> So khớp ngân sách
            </p>
          </div>
          <div className="mt-auto rounded-2xl border border-white/10 bg-white/5 p-4 text-xs leading-5 text-slate-400">
            <ShieldCheck className="mb-2 text-brand-400" size={20} />
            AI chỉ tìm kiếm và đề xuất. Bạn luôn là người xác nhận đặt sân và thanh toán.
          </div>
        </aside>
        ...
        <section className="flex h-[calc(100dvh-7rem)] min-h-[480px] max-h-[780px] min-w-0 flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-card sm:h-[76vh] sm:min-h-[560px] sm:rounded-3xl">
          <header className="flex items-center justify-between gap-3 border-b border-slate-100 px-4 py-4 sm:px-6">
            <div className="flex items-center gap-3">
              <span className="relative grid h-11 w-11 place-items-center rounded-2xl bg-ai-500 text-white">
                <Bot size={23} />
                <i className="absolute bottom-0 right-0 h-3 w-3 rounded-full border-2 border-white bg-emerald-300" />
              </span>
              <div>
                <h1 className="font-bold text-slate-950">AI Trợ lý SportHub</h1>
                <p className="text-xs text-slate-500">Dữ liệu sân & lịch trống theo thời gian thực</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              ...
              <Badge variant="success" className="hidden md:inline-flex" dot>
                Đang hoạt động
              </Badge>
            </div>
          </header>
            ...
            {messages.map((message) => (
              ...
              >
                ...
                <div className={`min-w-0 max-w-[94%] sm:max-w-[88%] ${message.role === 'user' ? 'order-first' : ''}`}>
                  ...
                  {message.role === 'assistant' && message.classification && message.classification !== 'IN_SCOPE' && (
              ...
              <div className="mb-3 flex gap-2 overflow-x-auto pb-1">
                {quickPrompts.map((prompt) => (
                  <button
                    ...
                    className="shrink-0 rounded-full border border-slate-200 bg-white px-3 py-2 text-xs font-medium text-slate-600 hover:border-brand-300 hover:text-brand-700"
                  >
                    {prompt}
                  </button>
                ))}
