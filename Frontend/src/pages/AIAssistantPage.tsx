...
import { AssistantApiError, AssistantTimeoutError, askSportHubAssistant, type AssistantIntent, type AssistantSuggestion } from '@/services/aiAssistantService';

const quickPrompts = [
  'Tối nay còn sân cầu lông nào?',
  'Tìm sân bóng dưới 700.000đ',
  'Sân pickleball ở Thủ Đức ngày mai',
  'Cầu lông lúc 17 giờ còn trống không?',
];

type Message = { id: number; role: 'assistant' | 'user'; text: string; suggestions?: AssistantSuggestion[]; classification?: 'IN_SCOPE' | 'OUT_OF_SCOPE' | 'UNCLEAR'; intent?: AssistantIntent };
...
export function AIAssistantPage() {
  ...
  const ask = async (value = query) => {
    ...
    try {
      ...
      if (response.classification !== 'OUT_OF_SCOPE') setSearchContext(response.understood);
      if (response.suggestions[0]) setContextFieldId(response.suggestions[0].field_id);
      setMessages((current) => [
        ...current.map((message) => message.suggestions ? { ...message, suggestions: undefined } : message),
        { id: Date.now() + 1, role: 'assistant', text: response.reply, suggestions: response.suggestions, classification: response.classification, intent: response.intent },
      ]);
    } catch (error) {
      ...
      setMessages((current) => [...current, { id: Date.now() + 1, role: 'assistant', text: errorText }]);
    } finally {
    ...
    <div className="mx-auto grid max-w-6xl gap-5 lg:grid-cols-[280px_minmax(0,1fr)]">
      ...
      <section className="flex h-[calc(100dvh-7rem)] min-h-[480px] max-h-[780px] min-w-0 flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-card sm:h-[76vh] sm:min-h-[560px] sm:rounded-3xl">
        ...
        <div ref={messagesRef} className="min-h-0 flex-1 space-y-5 overflow-y-auto bg-slate-50/60 p-4 sm:p-6">
          ...
          {loading && <div className="flex items-center gap-2.5"><span className="grid h-8 w-8 place-items-center rounded-xl bg-brand-600 text-white"><Bot size={16} /></span><div className="flex gap-1 rounded-2xl rounded-tl-md border bg-white px-4 py-4"><i className="h-2 w-2 animate-bounce rounded-full bg-slate-400" /><i className="h-2 w-2 animate-bounce rounded-full bg-slate-400 [animation-delay:120ms]" /><i className="h-2 w-2 animate-bounce rounded-full bg-slate-400 [animation-delay:240ms]" /></div></div>}
        </div>
        ...
        <footer className="border-t border-slate-100 bg-white p-3 sm:p-5">
          ...
          <form onSubmit={submit} className="flex items-end gap-2 overflow-hidden rounded-2xl border border-slate-200 bg-slate-50 p-2 focus-within:border-brand-400 focus-within:ring-2 focus-within:ring-brand-100">
            ...
            <textarea id="assistant-query" rows={1} value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); void ask(); } }} placeholder="Ví dụ: Tối nay còn sân cầu lông nào?" className="max-h-28 min-h-10 flex-1 resize-none rounded-xl bg-transparent px-2 py-2 text-sm outline-none focus:outline-none focus-visible:outline-none" />
...
function SuggestionCard({ item }: { item: AssistantSuggestion }) {
  return <article className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
    <div className="p-4">
      <div className="flex items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wide text-brand-700">{item.sport_type}</p><h2 className="mt-1 font-bold text-slate-950">{item.facility_name}</h2><p className="mt-0.5 text-xs font-medium text-slate-500">Sân: {item.court_name}</p></div>{item.is_nearest_alternative && <Badge variant="warning">Phương án gần nhất</Badge>}</div>
      ...
      <div className="mt-3 flex flex-wrap gap-2"><span className="rounded-lg bg-brand-50 px-2.5 py-1.5 text-xs font-semibold text-brand-700"><Clock3 size={13} className="mr-1 inline" />{item.start_time}–{item.end_time} · {dateLabel(item.booking_date)}</span><span className="rounded-lg bg-slate-100 px-2.5 py-1.5 text-xs font-bold text-slate-700">{money(item.price)}</span><span className="rounded-lg bg-amber-50 px-2.5 py-1.5 text-xs font-semibold text-amber-700"><Star size={12} className="mr-1 inline fill-current" />{item.rating.toFixed(1)}</span><Badge variant="success">Còn trống</Badge></div>
      <p className="mt-3 text-xs leading-5 text-slate-500">{item.reason}</p>
      <div className="mt-4 grid grid-cols-2 gap-2"><Link to={`/courts/${item.field_id}`}><Button variant="outline" className="w-full">Xem sân</Button></Link><Link to={`/booking/${item.field_id}?date=${item.booking_date}&slot=${item.time_slot_id}`}><Button className="w-full">Đặt sân <ChevronRight size={15} /></Button></Link></div>
    </div>
  </article>;
}
