...
export function SystemAdminPartnerApplicationsPage() {
  ...
  return <div className="min-h-screen bg-slate-50">
    ...
    {selected && <div data-scroll-lock="true" className="fixed inset-0 z-40 flex justify-end bg-slate-950/50" onMouseDown={() => !rejectOpen && setSelected(undefined)}>
      <aside className="h-[100dvh] w-full max-w-2xl overflow-y-auto overscroll-contain bg-white p-4 pb-36 shadow-2xl sm:p-6 sm:pb-28" onMouseDown={(event) => event.stopPropagation()}>
        <div className="flex items-start justify-between"><div className="min-w-0"><p className="text-xs font-bold uppercase text-brand-700">Yêu cầu #{selected.id}</p><h2 className="break-words text-xl font-black">{selected.customer_name}</h2></div><button type="button" onClick={() => setSelected(undefined)} className="shrink-0 rounded-lg p-2 hover:bg-slate-100" aria-label="Đóng"><X /></button></div>
        ...
        {selected.status === 'PENDING' && <div className="fixed bottom-0 right-0 flex w-full max-w-2xl flex-col gap-2 border-t bg-white px-4 pt-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] shadow-lg sm:flex-row sm:p-4"><Button className="w-full sm:w-auto" loading={working} onClick={() => void review('APPROVE')}>Duyệt và cấp OWNER</Button><Button className="w-full sm:w-auto" disabled={working} variant="danger" onClick={openRejectModal}>Từ chối</Button></div>}
      </aside>
    </div>}
