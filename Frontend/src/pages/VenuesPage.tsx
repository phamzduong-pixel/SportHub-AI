...
export function VenuesPage() {
  ...
  return <div className="mx-auto min-w-0 max-w-[1440px] px-3 py-5 min-[375px]:px-4 sm:px-6 sm:py-8">
    ...
    <div className="grid gap-6 lg:grid-cols-[270px_1fr]"><aside className="hidden h-fit rounded-card border border-slate-200 bg-white p-5 shadow-sm lg:block lg:sticky lg:top-20">{filtersUi}</aside><section className="min-w-0"><div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><p className="text-sm text-slate-500">Tìm thấy <b className="text-slate-900">{filtered.length}</b> sân phù hợp</p><div className="flex min-w-0 flex-wrap items-center gap-2"><Select aria-label="Sắp xếp" value={sort} onChange={(event) => setSort(event.target.value)} options={[{ label: 'Phù hợp nhất', value: 'recommended' }, { label: 'Giá thấp nhất', value: 'price' }, { label: 'Gần nhất', value: 'distance' }, { label: 'Đánh giá cao', value: 'rating' }]} className="w-full min-[360px]:w-44" /><div className="flex shrink-0 rounded-lg border border-slate-200 bg-white p-1"><button aria-label="Dạng lưới" onClick={() => setView('grid')} className={`rounded p-1.5 ${view === 'grid' ? 'bg-brand-50 text-brand-700' : 'text-slate-400'}`}><LayoutGrid size={17} /></button><button aria-label="Dạng danh sách" onClick={() => setView('list')} className={`rounded p-1.5 ${view === 'list' ? 'bg-brand-50 text-brand-700' : 'text-slate-400'}`}><List size={17} /></button><button aria-label="Bản đồ" onClick={() => setView('map')} className={`rounded p-1.5 ${view === 'map' ? 'bg-sportblue-50 text-sportblue-600' : 'text-slate-400'}`}><Map size={17} /></button></div></div></div>
