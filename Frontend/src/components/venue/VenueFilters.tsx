...
import { getSportProductOptions } from '@/services/productService';
...
export function VenueFilters({ value, onChange, onReset, onApply }: { value: VenueFilterState; onChange: (next: VenueFilterState) => void; onReset: () => void; onApply?: () => void }) {
  ...
  useEffect(() => {
    ...
    setOptionsError('');
    getSportProductOptions(value.sport)
      .then((opts) => {
        if (active) {
      ...
      .catch((err) => {
        console.error('Error fetching sport amenities:', err);
        if (active) {
          setProductOptions([]);
          setOptionsError('Không tải được danh sách tiện ích. Vui lòng thử lại.');
        }
      })
      .finally(() => {
  ...
  return <div className="min-w-0 space-y-5">
    ...
    <Select
      ...
      onChange={(event) => {
        ...
        onChange({ ...value, sport: nextSport, amenities: [] });
      }}
      options={['Bóng đá', 'Cầu lông', 'Pickleball', 'Tennis', 'Bóng rổ', 'Bóng chuyền'].map((item) => ({ label: item, value: item }))}
    ...
    <fieldset>
      <legend className="mb-3 text-sm font-medium text-slate-700">Tiện ích</legend>
      {!value.sport ? (
        <p className="text-xs text-slate-500 italic">Vui lòng chọn môn thể thao để xem tiện ích/dịch vụ.</p>
      ) : loadingOptions ? (
        ...
        <p className="text-xs text-red-500 italic">{optionsError}</p>
      ) : productOptions.length === 0 ? (
        <p className="text-xs text-slate-500 italic">Chưa có tiện ích phù hợp cho môn này.</p>
      ) : (
        <div className="grid grid-cols-1 gap-2 min-[360px]:grid-cols-2">
          {productOptions.map((amenity) => (
            <label key={amenity} className="flex min-w-0 cursor-pointer items-start gap-2 rounded-lg border border-slate-200 p-2.5 text-xs text-slate-600 hover:bg-slate-50">
              <input
                type="checkbox"
