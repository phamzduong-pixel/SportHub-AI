...
export function AuthGuard({ children }: { children: ReactNode }) { const { user, loading } = useAuth(); const location = useLocation(); if (loading) return <Pending />; return user ? children : <Navigate to="/login" replace state={{ from: `${location.pathname}${location.search}` }} />; }
...
export function OwnerVerificationGuard({ children }: { children: ReactNode }) { const { user, loading } = useAuth(); if (loading) return <Pending />; return user?.role === 'OWNER' ? children : <Navigate to="/login" replace />; }
