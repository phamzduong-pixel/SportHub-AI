...
export function Modal({ open, onClose, title, description, children }: Props) {
  ...
  if (!open) return null;
  return createPortal(
    <div className="fixed inset-0 z-50 grid place-items-center overflow-hidden bg-slate-950/45 p-2 sm:p-4" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <section role="dialog" aria-modal="true" aria-labelledby="modal-title" className="my-auto flex max-h-[calc(100dvh-1rem)] w-full min-w-0 max-w-lg flex-col overflow-hidden rounded-2xl bg-white shadow-float sm:max-h-[calc(100dvh-2rem)] sm:rounded-card">
