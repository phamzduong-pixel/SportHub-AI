...
export interface BookingSlotSnapshot { time_slot_id: number; name: string; start_time: string; end_time: string; price: number; }
export interface BookingProductSelection { product_id: number; quantity: number; }
export interface BookingProductSnapshot { item_id: number | null; product_id: number; name: string; product_type: 'SELL' | 'RENT' | 'SERVICE'; unit: string; quantity: number; unit_price: number; subtotal: number; inventory_status?: string | null; source: 'CUSTOMER_BOOKING' | 'OWNER_DURING_USAGE'; added_by: number | null; added_by_name: string | null; added_at: string | null; }
export interface ApiBooking {
  ...
  facility_id: number | null; facility_name: string; facility_hotline: string | null; field_id: number; field_name: string; sport_type: string; field_capacity: number; location: string; time_slot_id: number; time_slot_ids: number[]; selected_slots: BookingSlotSnapshot[];
  time_slot_name: string; booking_date: string; start_time_snapshot: string; end_time_snapshot: string;
  price_snapshot: number; court_amount: number; service_amount: number; product_items: BookingProductSnapshot[]; total_amount: number; deposit_type: 'percentage' | 'fixed'; deposit_value: number;
...
export interface BookingQuote { venue_id: number | null; venue_name: string; field_id: number; field_name: string; sport_type: string; field_type: string; location: string; time_slot_id: number; time_slot_ids: number[]; selected_slots: BookingSlotSnapshot[]; time_slot_name: string; booking_date: string; start_time: string; end_time: string; duration_minutes: number; price: number; court_amount: number; service_amount: number; product_items: BookingProductSnapshot[]; total_amount: number; deposit_amount: number; remaining_amount: number; deposit_type: string; deposit_value: number; hold_minutes: number; free_cancellation_minutes: number; cancellation_policy_summary: string; }
export interface InvoiceInfo { invoice_number: string; transaction_code: string; booking_code: string; customer_name: string; customer_email: string; field_name: string; facility_name: string; booking_date: string; court_amount: number; service_amount: number; product_items: BookingProductSnapshot[]; total_amount: number; deposit_amount: number; remaining_payment_amount: number; paid_amount: number; remaining_amount: number; payment_method: string; bank_name: string | null; paid_at: string; }
export interface BookingInvoice { invoice_number: string; booking_id: number; booking_code: string; customer_name: string; customer_email: string; facility_name: string; field_name: string; booking_date: string; start_time: string; end_time: string; duration_minutes: number; selected_slots: BookingSlotSnapshot[]; court_amount: number; service_amount: number; product_items: BookingProductSnapshot[]; total_amount: number; deposit_amount: number; remaining_payment_amount: number; refund_amount: number; net_received_amount: number; payment_methods: string; paid_at: string | null; issued_at: string; }
export interface ApiPayment { id: number; booking_id: number; booking_code: string; customer_id: number; owner_id: number; court_amount: number; service_amount: number; product_items: BookingProductSnapshot[]; transaction_code: string; amount: number; total_amount: number; deposit_amount: number; remaining_amount: number; paid_amount: number; payment_status: string; payment_method: string; payment_type: 'deposit' | 'remaining' | 'full' | 'refund'; status: string; escrow_status: 'pending' | 'held' | 'released' | 'refunded' | 'failed'; paid_at: string | null; refunded_at: string | null; failed_reason: string | null; provider: string | null; bank_id: string | null; bank_name: string | null; bank_account_no: string | null; bank_account_name: string | null; transfer_content: string | null; qr_url: string | null; expires_at: string | null; provider_reference: string | null; verification_source: string | null; refund_status: string; payment_mode: 'demo' | 'production'; invoice: InvoiceInfo | null; created_at: string; updated_at: string; }
export interface DepositReceiptData { receipt_number: string; booking_id: number; booking_code: string; customer_name: string; facility_name: string; facility_address: string; field_name: string; sport_type: string; booking_date: string; start_time: string; end_time: string; duration_minutes: number; selected_slots: BookingSlotSnapshot[]; court_amount: number; service_amount: number; product_items: BookingProductSnapshot[]; total_amount: number; deposit_paid: number; remaining_amount: number; transaction_code: string; payment_method: string; bank_name: string | null; paid_at: string; booking_status: string; deposit_status: 'paid' | 'paid_pending_confirmation' | 'refund_pending' | 'refunded'; status_message: string; refund_status: string; refund_amount: number; refunded_at: string | null; }
...
export interface RescheduleRequest { field_id: number; time_slot_id: number; time_slot_ids: number[]; booking_date: string; }
...
export const getBookingQuote = (fieldId: number, slotIds: number[], date: string, productItems: BookingProductSelection[] = []) => {
  const params = new URLSearchParams({ field_id: String(fieldId), date });
  slotIds.forEach((id) => params.append('time_slot_ids', String(id)));
  productItems.forEach((item) => {
    params.append('product_id', String(item.product_id));
    params.append('product_quantity', String(item.quantity));
  });
  return apiRequest<BookingQuote>('/bookings/quote?' + params.toString());
};
