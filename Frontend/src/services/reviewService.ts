...
export const getReviewByBooking = (bookingId: number) => apiRequest<Review>(`/bookings/${bookingId}/review`);
export const getCustomerReviews = () => apiRequest<Review[]>('/customer/reviews');
...
export const updateReview = (id: number, rating: number, comment: string) => apiRequest<Review>(`/reviews/${id}`, { method: 'PATCH', body: JSON.stringify({ rating, comment }) });
